...
interface Statement {
    type: 'if' | 'return' | 'throw' | 'try' | 'foreach' | 'for' | 'while' | 'await' | 'statement';
    text: string;
    condition?: string;
    trueBranch?: string;
    falseBranch?: string;
...
export class FlowchartProvider {

    public static showFlowchartForCurrentMethod(): void {
    ...
    private static _extractMethodAtOffset(
        ...
        let best: { name: string; bodyStart: number; sigEnd: number } | null = null;
        ...
        if (offset < best.bodyStart || offset > i) return null;
    ...
    private static _buildMermaid(methodName: string, body: string): string {
        ...
        for (const s of stmts) {
            ...
            if (s.type === 'return' || s.type === 'throw') {
                ...
                // generic statement
                if (!s.text || s.text.length < 2) continue;
                lines.push(`    ${nid}["${this._esc(s.text)}"]`);
                lines.push(`    ${prev} --> ${nid}`);
                prev = nid;
            }
    ...
    private static _parseStatements(body: string): Statement[] {
        const out: Statement[] = [];
        ...
        for (let i = 0; i < rawLines.length; i++) {
            ...
            if (!raw || raw.startsWith('//') || raw.startsWith('*') || raw === '{' || raw === '}') continue;
            ...
            if (ifM) {
                ...
                const innerFalse = hasElse
                    ? (rawLines.slice(i + 1, i + 8).find(l => {
                        const t = l.trim(); return t && t !== '{' && t !== '}' && !/^(if|else|\/\/)/.test(t);
                    })?.trim().replace(/[;{}]/g, '') ?? 'else')
                    : undefined;
    ...
    private static _getHtml(methodName: string, mermaid: string): string {
        return `<!DOCTYPE html>
<html lang="sl">
<head>
    <meta charset="UTF-8">
