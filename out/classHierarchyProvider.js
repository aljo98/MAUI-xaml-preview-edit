...
exports.ClassHierarchyProvider = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
class ClassHierarchyProvider {
    static async showForCurrentFile() {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: `Gradim hierarhijo za ${targetName}…` }, async () => {
            ...
            const panel = vscode.window.createWebviewPanel('mauiClassHierarchy', `Hierarhija: ${targetName}`, vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true });
            panel.webview.html = this._getHtml(targetName, mermaid);
        });
    }
    // ── Parser ──────────────────────────────────────────────────────────────────
    static _parseFile(filePath, content) {
        const results = [];
    ...
    // ── Mermaid ─────────────────────────────────────────────────────────────────
    static _buildMermaid(target, all) {
        ...
        const addChildren = (name, depth = 0) => {
            if (depth > 3)
                return; // max depth
    ...
    // ── HTML ────────────────────────────────────────────────────────────────────
    static _getHtml(className, mermaid) {
        return `<!DOCTYPE html>
<html lang="sl">
<head>
    <meta charset="UTF-8">
...
exports.ClassHierarchyProvider = ClassHierarchyProvider;
//# sourceMappingURL=classHierarchyProvider.js.map