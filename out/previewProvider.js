...
class MauiXamlPreviewProvider {
     ...
     */
    _notifyMessageCallbacks(msg) {
        for (const cb of this._messageCallbacks) {
            try {
                cb(msg);
            }
            catch (e) {
                console.error('[PreviewProvider] Callback error:', e);
            }
        }
    }
    /**
     ...
     */
    getElementRange(elementId) {
        const info = this._elementMap.get(elementId);
        if (info)
            return { startLine: info.startLine, endLine: info.endLine };
    ...
    async selectElementByCode(name, isCommand) {
    ...
    setPropertiesWebviewProvider(provider) {
     ...
     */
    _getHeuristicSampleValue(propName) {
        const lower = propName.toLowerCase();
        if (lower.includes('number') || lower.includes('code')) {
            return 'OR-2025-001';
        }
        if (lower.includes('customername') || lower.includes('partnername')) {
            return 'Acme Corp d.o.o.';
        }
        ...
        if (lower.includes('phone') || lower.includes('tel')) {
            return '+386 1 234 5678';
        }
        if (lower.includes('grandtotal') || lower.includes('grand_total')) {
            return '1250.00';
        }
        if (lower.includes('total') || lower.includes('amount') || lower.includes('vsota')) {
            return '1250.00';
        }
        if (lower.includes('price') || lower.includes('cena')) {
            return '125.00';
        }
        ...
        if (lower.includes('person') || lower.includes('prodajalec')) {
            return 'Janez Novak';
        }
        if (lower.includes('address') || lower.includes('naslov')) {
            return 'Slovenska 1, Ljubljana';
        }
        if (lower.includes('note') || lower.includes('opomba')) {
            return 'Opomba...';
        }
        if (lower.includes('description') || lower.includes('opis')) {
            return 'Opis...';
        }
        if (lower.includes('display') || lower.includes('label')) {
            return propName;
        }
        if (lower.includes('error') || lower.includes('message')) {
            return '';
        }
        if (lower.includes('text') || lower.includes('search')) {
            return '';
        }
     ...
     */
    _extractDesignTimeDataFromCs(csContent) {
        if (!csContent) {
            return;
        }
        let m;
        ...
        while ((m = staticArrRe.exec(csContent)) !== null) {
            ...
            if (items.length > 0 && !this._designTimeData.has(`${pName}__items`)) {
                ...
                if (!this._designTimeData.has(pName)) {
                    this._designTimeData.set(pName, items[0]);
                }
            }
        }
        ...
        while ((m = strFieldRe.exec(csContent)) !== null) {
            ...
            if (!this._designTimeData.has(pName)) {
                this._designTimeData.set(pName, m[2]);
            }
        }
        ...
        while ((m = boolFieldRe.exec(csContent)) !== null) {
            const pName = this._toPascalCase(m[1]);
            if (!this._designTimeData.has(pName)) {
                this._designTimeData.set(pName, m[2] || 'false');
            }
        }
        ...
        while ((m = boolComputedRe.exec(csContent)) !== null) {
            if (!this._designTimeData.has(m[1])) {
                this._designTimeData.set(m[1], 'false');
            }
        }
     ...
     */
    async _loadDesignTimeData(document) {
     ...
     */
    _resolveBindingValue(propName, formatStr) {
        let value = this._designTimeData.get(propName);
        if (value === undefined) {
        ...
        const result = fmt
            .replace(/\{0:N(\d+)\}/ig, (_, d) => numVal.toLocaleString('de-DE', { minimumFractionDigits: Number(d), maximumFractionDigits: Number(d) }))
            .replace(/\{0:F(\d+)\}/ig, (_, d) => numVal.toFixed(Number(d)))
    ...
    _generatePreviewHtml() {
        ...
        if (/\/Sales\/|\/Prodaja\//i.test(fp)) {
            activeModule = 'Prodaja';
        }
        else if (/\/Purchases\/|\/Nabava\//i.test(fp)) {
            activeModule = 'Nabava';
        }
        else if (/\/CRM\//i.test(fp)) {
            activeModule = 'CRM';
        }
        else if (/\/Finance\//i.test(fp)) {
            activeModule = 'Finance';
        }
        else if (/\/Inventory\/|\/Warehouse\/|\/Skladi/i.test(fp)) {
            activeModule = 'Skladišče';
        }
        else if (/\/Manufacturing\/|\/Proizvodnja\//i.test(fp)) {
            activeModule = 'Proizvodnja';
        }
        else if (/\/Projects\/|\/Projekti\//i.test(fp)) {
            activeModule = 'Projekti';
        }
        else if (/\/Quality\/|\/Kakovost\//i.test(fp)) {
            activeModule = 'Kakovost';
        }
        else if (/\/Service\/|\/Servis\//i.test(fp)) {
            activeModule = 'Servis';
        }
        else if (/\/Products\/|\/Izdelki\//i.test(fp)) {
            activeModule = 'Izdelki';
        }
        else if (/\/HRM\/|\/Kadri\//i.test(fp)) {
            activeModule = 'HRM';
        }
        ...
        const sa = (mod) => activeModule === mod ? ' data-active="true"' : '';
            ...
            case 'Picker': {
                ...
                if (itemsSource) {
                    ...
                    if (xStaticMatch) {
                        ...
                        if (itemsJson) {
                            try {
                                pickerItems = JSON.parse(itemsJson);
                            }
                            catch (_) { }
                        }
                    }
